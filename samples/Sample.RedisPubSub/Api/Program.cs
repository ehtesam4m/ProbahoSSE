...
// ── ProbahoSSE with Redis Pub/Sub backplane ──────────────────────────────────
builder.Services.AddProbahoSse(options =>
    ...
    .AddRedisPubSubBackplane(redis =>
    {
        ...
        redis.ChannelPrefix = "probaho-pubsub";
    });
...
// Health check
app.MapGet("/health", () => Results.Ok(new { status = "healthy", instance = Environment.MachineName, backplane = "RedisPubSub" }));

// Info endpoint — used by the HTML demo to auto-detect which backplane is running
app.MapGet("/info", () => Results.Ok(new
{
    backplane = "RedisPubSub",
